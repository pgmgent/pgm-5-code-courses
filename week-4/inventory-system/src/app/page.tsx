...
export default function Home() {
  ...
  const sortedData = filteredData.sort((a, b) => {
    ...
    return !a[sortField as keyof typeof a] < !b[sortField as keyof typeof b]
      ? -order
      : order;
  });
  ...
  return (
    <div className="p-4 space-y-4">
      <h1 className="text-2xl font-bold">
        Uitgebreid inventarisbeheer voor hout en glazen potten
      </h1>
      <input
        type="text"
      ...
      <ul className="space-y-2">
        {sortedData.map((item) => (
          <li key={item.id} className="flex justify-between items-center">
            <span>
              Item: {item.item}, Voorraad: {item.voorraad}
            </span>
            <div className="space-x-2">
              ...
              <button
                ...
                onClick={() => purchaseItem(item.id, 5)}
              >
                Inkoop +5
              </button>
              <button
                className="bg-green-500 text-white px-2 py-1 rounded-lg"
                onClick={() => sellItem(item.id, 5)}
              >
                Verkoop -5
              </button>
            </div>
      ...
      <div className="text-center">
        <input
          ...
          className="rounded-lg border-solid border-2 border-indigo-500 px-3 py-1"
        />
        <button
          className="bg-indigo-500 text-white px-3 py-2 rounded-lg"
          onClick={() => addItem(newItem, 10)}
        >
          Voeg nieuw item toe
        </button>
      </div>
