...
const ProductUploadForm = ({userId}: {userId:string}) => {
  ...
  const [createProduct, createProductState] = useMutation(CREATE_PRODUCT_MUTATION);
  ...
  const uploadMedia = async (file: File): Promise<string> => {
    ...
    try {
      ...
      console.log("uploadMediaState", uploadMediaState)
      const { data } = uploadMediaState;
  ...
  if (uploadMediaState.error && createProductState.error) return <div>failed to load</div>
  return (
